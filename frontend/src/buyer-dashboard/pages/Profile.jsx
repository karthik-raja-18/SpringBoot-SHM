...
const BuyerProfile = () => {
  ...
  const fetchProfile = async () => {
    try {
      ...
      if (err.response?.status === 403) {
        setError('You do not have permission to access this page. Please log in as a buyer.');
  ...
  const fetchOrders = async () => {
    try {
      ...
      // Don't block profile for orders error
    }
  };
  ...
  const fetchWishlist = async () => {
    try {
      ...
      // Don't block profile for wishlist error
    }
  };
  ...
  const fetchCart = async () => {
    try {
      ...
      // Don't block profile for cart error
    }
  };
      ...
      <div className="profile-container">
        ...
        <div className="profile-sidebar">
          ...
          <div className="profile-tips">
            <h3>Profile Tips</h3>
            <ul>
              <li>Keep your contact information up to date for smooth order processing</li>
              <li>Check your wishlist and cart regularly for deals</li>
      ...
      <div className="profile-extras">
          ...
          {orders.length === 0 ? (
            ...
            <ul>
              {(Array.isArray(orders) ? orders.slice(0, 3) : []).map(order => (
                <li key={order.id}>
                  Order #{order.id} - Status: {order.status} - Date: {order.createdAt || order.orderDate}
                </li>
              ))}
            </ul>
          )}
        ...
        <div className="wishlist-summary">
          ...
          {wishlist.length === 0 ? (
            ...
            <ul>
              {(Array.isArray(wishlist) ? wishlist.slice(0, 3) : []).map(item => (
                <li key={item.id || item._id}>{item.title || item.name}</li>
              ))}
            </ul>
          )}
        </div>
        <div className="cart-summary">
          ...
          {cart.length === 0 ? (
            ...
            <ul>
              {(Array.isArray(cart) ? cart.slice(0, 3) : []).map(item => (
                <li key={item.id || item._id}>{item.title || item.name}</li>
              ))}
            </ul>
          )}
